...
export const updatePassword = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!req.body.password || !req.body.passwordConfirm) {
        return next(new AppError('Please provide a new password and passwordConfirm', 400));
    }
...
export const confirmEmail = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user?.emailConfirmExpires && user.emailConfirmExpires < new Date()) {
        ...
        return next(new AppError('Token is invalid or has expired, please sign up again', 400));
    }
